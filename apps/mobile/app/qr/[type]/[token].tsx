...
import { formatOrderLabel, type OrderStatus } from "@/lib/order-display";
...
export default function QrResolveScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (payload.ok && payload.type === "order" && payload.orderNumber) {
        router.replace({
          pathname: "/order/[id]",
          params: {
  ...
  const pay = useCallback(async () => {
    ...
    try {
      ...
      if (sheet.type !== "completed") {
        throw new Error(sheet.type === "failed" || sheet.type === "init_failed" ? sheet.message : "Payment failed.");
      }
  ...
  return (
    <ScreenContainer>
      <View style={styles.flex}>
        <Header title="QR code" showBack showSearch={false} showCart={false} />
        {isOffline ? (
          ...
          <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + spacing["2xl"] }]}>
            ...
            <Text style={styles.body}>
              The QR code only opens this payment page. Payment is confirmed by Stripe after checkout.
            </Text>
          ...
          <ScrollView contentContainerStyle={styles.content}>
            <Text style={styles.kicker}>Track Order</Text>
            <Text style={styles.title}>{result.order.orderNumber}</Text>
            <Text style={styles.body}>
              {formatOrderLabel(result.order.status)}
            </Text>
            <OrderProgressTimeline status={result.order.status as OrderStatus} />
          </ScrollView>
        ) : (
          <EmptyState
...
function createStyles({ colors, textStyles }: ThemeStyleTokens) {
  return {
    ...
    amount: { ...textStyles.sectionTitle, color: colors.primary },
