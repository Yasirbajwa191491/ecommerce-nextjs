...
export function ProductCatalogFilters({
  ...
  return (
    <aside
      className={cn(
        "flex flex-col gap-2 rounded-xl border border-border/60 bg-card p-2 shadow-sm sm:gap-4 sm:rounded-2xl sm:p-4 md:gap-5 md:p-5 lg:min-h-[calc(100vh-11rem)] lg:gap-6 lg:p-6 lg:shadow-sm",
        className
      )}
    >
      <div className="flex items-center gap-1.5 sm:gap-2.5">
        <SlidersHorizontal className="size-3.5 shrink-0 text-[#6254f3] sm:size-4" />
        <h2 className="text-[10px] font-semibold tracking-tight text-foreground sm:text-base">
          Filters
        </h2>
      </div>
      ...
      <div className="space-y-2 sm:space-y-3 lg:space-y-4">
        <p className="text-[9px] font-semibold tracking-wide text-muted-foreground uppercase sm:text-xs">
          Category
        </p>
      ...
      <div className="space-y-2 sm:space-y-3 lg:space-y-5">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
          ...
          <span className="w-fit rounded-full bg-muted px-1 py-0.5 text-[8px] font-medium text-foreground tabular-nums sm:px-2.5 sm:py-1 sm:text-[11px]">
      ...
      <Button
        ...
        variant="outline"
        onClick={onClear}
        className="mt-auto h-7 w-full text-[10px] sm:h-9 sm:text-sm lg:mt-auto"
      >
        Clear filters
      </Button>
    </aside>
...
function CategoryOption({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "flex w-full min-w-0 items-start gap-1 rounded-md px-1 py-1 text-left text-[9px] leading-snug capitalize transition-colors sm:items-center sm:gap-2 sm:rounded-lg sm:px-2.5 sm:py-2 sm:text-xs md:gap-3 md:px-3 md:py-2.5 md:text-sm",
        checked
    ...
    >
      <span
        className={cn(
          "mt-0.5 size-2.5 shrink-0 rounded-full border-2 sm:mt-0 sm:size-3.5 md:size-4",
          checked ? "border-[#6254f3] bg-[#6254f3]" : "border-border bg-background"
        )}
        aria-hidden
      />
      <span className="min-w-0 flex-1 break-words whitespace-normal sm:truncate sm:whitespace-nowrap">
        {label}
      </span>
    </button>
  );
